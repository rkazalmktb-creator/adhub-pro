...
export function formatDateTime(dateString: string): string {
  if (!dateString) return '—';
  try {
    return new Date(dateString).toLocaleString('ar-LY-u-nu-latn');
  } catch {
    return dateString;
  }
}
...
export function formatHijriDate(dateString: string): string {
  ...
  try {
    ...
    return new Intl.DateTimeFormat('ar-u-ca-islamic-umalqura-nu-latn', {
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    }).format(date) + ' هـ';
...
/**
 * تنسيق التاريخ مع الهجري (إذا مفعّل)
 */
export function formatDateWithHijri(dateString: string, showHijri: boolean, locale: string = 'ar-LY-u-nu-latn'): string {
  if (!dateString) return '—';
  try {
    const date = new Date(dateString);
