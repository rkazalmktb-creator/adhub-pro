...
const generateHeader = (config: PrintConfig, documentData: PrintDocumentData, headerSwap?: boolean): string => {
  ...
  // ✅ تبديل ترتيب النصفين حسب header_swap
  return `
    <div class="measurements-header${headerSwap ? ' measurements-header-swapped' : ''}">
      ${headerSwap ? companySide + titleSide : titleSide + companySide}
    </div>
  `;
};
...
export const generateMeasurementsCSS = (config: PrintConfig): string => {
  ...
  return `
    ...
    * { 
    ...
    /* Swapped header - when logo goes to left, title to right */
    .measurements-header-swapped {
