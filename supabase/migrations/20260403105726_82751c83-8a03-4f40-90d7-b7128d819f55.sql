...
-- إضافة عمود الترتيب/الأولوية لجدول فرق التركيب
ALTER TABLE installation_teams ADD COLUMN IF NOT EXISTS priority integer NOT NULL DEFAULT 0;

-- تحديث trigger التوزيع التلقائي ليستخدم الأولوية
...
BEGIN
  ...
  IF is_update THEN
    ...
    IF new_billboard_ids IS NOT NULL THEN
      -- توزيع كل لوحة جديدة على الفريق الأنسب (الأعلى أولوية)
      FOR bb IN
        SELECT b."ID", b."Size", b."City", COALESCE(b."Faces_Count", 2) as faces
        FROM billboards b
        WHERE b."ID" = ANY(new_billboard_ids)
      LOOP
        -- البحث عن أفضل فريق (مقاس + مدينة + أعلى أولوية)
        ...
        IF best_team_id IS NOT NULL THEN
          -- البحث عن مهمة موجودة لهذا الفريق والعقد
        ...
        END IF;
        -- إذا لم يوجد فريق مناسب، اللوحة لا تُسند
      END LOOP;
    END IF;
  ...
  ELSE
    -- إنشاء أولي: توزيع كل لوحة على الفريق الأنسب
    FOR bb IN
      SELECT b."ID", b."Size", b."City", COALESCE(b."Faces_Count", 2) as faces
      FROM billboards b
